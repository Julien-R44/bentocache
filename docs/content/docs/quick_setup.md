...
- We are using the `getOrSet` method. This method will first try to fetch the user from the cache. If it is not found, it will execute the _factory_ and store the result in the cache for 5 minutes.
